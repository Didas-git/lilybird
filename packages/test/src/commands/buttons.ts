...
import { CollectorType } from "@lilybird/transformers";
    ...
    run: async (interaction) => {
        ...
        switch (interaction.data.getInteger("type", true) as CollectorType) {
            case CollectorType.USER: {
                interaction.createComponentCollector(CollectorType.USER, interaction.member.user.id, async (int) => {
            ...
            case CollectorType.BUTTON_ID: {
                interaction.createComponentCollector(CollectorType.BUTTON_ID, "custom_test", async (int) => {
            ...
            case CollectorType.BOTH: {
                interaction.createComponentCollector(CollectorType.BOTH, interaction.member.user.id, "custom_test", async (int) => {
                    if (!int.data.isButton()) throw new Error("This should have never happened");
