...
Lilybird provides a lot of packages that can bring extra functionality and QoL features to your bot.

- `lilybird` - The core itself and all you need to make your bot.
- `@lilybird/jsx` - JSX components for lilybird using `react-jsx`.
- `@lilybird/handlers` - Command and Event handlers for lilybird.
- `@lilybird/transformers` - High level apis for lilybird.

There is also `@lilybird/create` which is the boilerplate generator used in the [getting started page](/guides/getting-started).
