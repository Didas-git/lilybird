...
export type ApplicationCommandHandler<O extends Array<CommandOption>, HO extends (...args: any) => any> = HO extends never
...
export type ApplicationCommandAutocompleteHandler<O extends Array<CommandOption>, AO extends (...args: any) => any> = AO extends never
    ? (client: Client, interaction: Interaction.ApplicationCommandInteractionStructure, options: Expand<ParseOptions<O>>) => Awaitable<unknown>
    : AO;
...
interface CompiledCommand {
    ...
    function: { names: Array<string>, handlers: Array<ApplicationCommandHandler<Array<CommandOption>, any> | ApplicationCommandAutocompleteHandler<Array<CommandOption>, any>> };
    // eslint-disable-next-line @typescript-eslint/naming-convention
    json: ApplicationCommand.Create.ApplicationCommandJSONParams & { __meta?: { ids?: string } };
}

export interface CommandStructure<
    O extends Array<CommandOption>,
    HO extends (...args: any) => any,
    AO extends (...args: any) => any
> extends Omit<ApplicationCommand.Create.ApplicationCommandJSONParams, "options"> {
    options?: O;
    meta?: CommandMeta;
...
export class ApplicationCommandStore<HO extends (...args: any) => any, AO extends (...args: any) => any> {
    ...
    public storeCommand<const O extends Array<CommandOption>>(command: CommandStructure<O, HO, AO>): void {
    ...
    #makeCommandBase(
        ...
        handler: { base_executor: ApplicationCommandHandler<Array<CommandOption>, HO>, auto_executor?: ApplicationCommandAutocompleteHandler<Array<CommandOption>, AO> },
        useElse: boolean,
    ...
    #compileCommand(
        command: ApplicationCommand.Create.ApplicationCommandJSONParams,
        options: Required<CommandStructure<Array<CommandOption>, HO, AO>>["options"],
        useElse: boolean,
        optionsBody: string,
        matchTo: "interaction_name" | "sub_command" | "sub_command_group" = "interaction_name",
        name: string = command.name
    ): CompiledCommand {
        const fns = new Map<string, ApplicationCommandHandler<Array<CommandOption>, HO> | ApplicationCommandAutocompleteHandler<Array<CommandOption>, AO>>();
        ...
        for (let i = 0, { length } = options; i < length; i++) {
            ...
            if (subCommand.type === ApplicationCommandOptionType.SUB_COMMAND_GROUP) {
                ...
                const { handle, autocomplete, ...realCommand } = <SubCommandStructure<Array<CommandOption>, HO, AO>>subCommand;
    ...
    #isCommandWrapper(options: Required<CommandStructure<Array<CommandOption>, HO, AO>>["options"]): boolean {
        for (let i = 0, { length } = options; i < length; i++) {
    ...
    public getCompilationStack(): {
        ...
        const functions = new Map<string, ApplicationCommandHandler<Array<CommandOption>, HO> | ApplicationCommandAutocompleteHandler<Array<CommandOption>, AO>>();
